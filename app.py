...
APP_TITLE = "CloudVision Web V4.2｜三眼視力、登入統計與Excel同步穩定版"
...
class FullscreenAcuityChart:
    ...
    def _save_public_result(self, payload: dict, client_ip: str, user_agent: str) -> dict:
        ...
        # 後端再次驗證三項視力，避免錯字或超出範圍的值寫入 Excel。
        valid_va_values = {
            "0.10", "0.13", "0.16", "0.20", "0.25", "0.32", "0.40",
            "0.50", "0.63", "0.79", "1.00", "1.26", "1.50",
        }
        for va_key in ("visual_acuity_right", "visual_acuity_left", "visual_acuity_both"):
            raw_va = clean.get(va_key, "")
            if raw_va:
                try:
                    normalized_va = f"{float(raw_va):.2f}"
                except (TypeError, ValueError):
                    normalized_va = ""
                clean[va_key] = normalized_va if normalized_va in valid_va_values else ""

        # 舊瀏覽器若沒有帶入 session_id，建立本次專用值，避免多次送出時互相覆蓋。
        if not clean.get("session_id"):
            clean["session_id"] = "result-" + now.strftime("%Y%m%d%H%M%S") + "-" + secrets.token_hex(4)
